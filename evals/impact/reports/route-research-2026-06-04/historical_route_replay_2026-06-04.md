...
- Generated at: `2026-06-04T20:12:36+10:00`
...
- Replayed traces: `57`
- Unmatched traces: `0`
- Confidence counts: `{'high': 57}`
- Route command candidates scanned: `716`

## Comparison

| cohort | traces | precision | noise | selected | selected_useful |
| --- | --- | --- | --- | --- | --- |
| baseline | 57 | 0.535 | 0.465 | 333 | 178 |
| replay | 57 | 0.350 | 0.650 | 320 | 112 |
...
- Filtered future docs: `479`
- Selected future docs: `0`
- Selected unknown-created-at docs: `77`
...
| --- | --- | --- | --- | --- | --- |
| promote-draft-ai-wiki-people-bochengyin-index-md | high | 0.167 | 0.167 | 0.000 | 检查是否已有自动 promote draft 到 ai-wiki/people/bochengyin/index.md 的机制 |
...
| evaluate-another-agent-s-assessment-that-ai-wiki-toolkit-has-first-version-workf | high | 0.667 | 0.667 | 0.000 | Evaluate another agent's assessment that ai-wiki-toolkit has first-version workflow eva... |
...
| decide-the-next-implementation-step-after-adding-eval-impact-manifest-and-eval-i | high | 0.500 | 0.167 | -0.333 | Decide the next implementation step after adding eval impact manifest and eval impact p... |
...
| implement-the-three-step-autonomous-impact-eval-runner-v0-single-slot-run-v1-all | high | 1.000 | 0.667 | -0.333 | Implement the three-step autonomous impact eval runner: v0 single-slot run, v1 all-slot... |
...
| clarify-whether-users-must-run-plan-prepare-run-report-separately-or-whether-the | high | 0.333 | 0.167 | -0.167 | Clarify whether users must run plan prepare run report separately or whether the curren... |
...
| build-a-continuous-impact-eval-loop-automatically-discover-new-eval-families-fro | high | 1.000 | 0.667 | -0.333 | Build a continuous impact eval loop: automatically discover new eval families from tria... |
...
| merge-the-source-incident-timing-feature-into-the-repo-release-the-npm-package-u | high | 0.667 | 0.333 | -0.333 | Merge the source incident timing feature into the repo, release the npm package, update... |
...
| ai-wiki-toolkit-trial-and-error-source-incident | high | 0.667 | 0.500 | -0.167 | 用户询问 ai-wiki-toolkit 被用户下载后，是否可以自动沉淀 trial and error / source incident 经验 |
...
| push-current-changes-to-remote-merge-the-pr-then-release-npm-for-ai-wiki-toolkit | high | 0.333 | 0.333 | 0.000 | push current changes to remote, merge the PR, then release npm for ai-wiki-toolkit |
| ai-wiki-toolkit-index-indexing | high | 0.000 | 0.167 | 0.167 | 那我们的 ai wiki toolkit 里面的 index 是不是就是 indexing 呢？ |
...
| redacted-external-form-task | high | 0.667 | 0.000 | -0.667 | Redacted external form-writing prompt |
